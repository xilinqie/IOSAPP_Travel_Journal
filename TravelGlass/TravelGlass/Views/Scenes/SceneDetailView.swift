...
struct SceneDetailView: View {
    ...
    @State private var showingAddToSet = false
    ...
    var body: some View {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    ScrollView(.horizontal, showsIndicators: false) {
                        HStack(spacing: 12) {
                            ...
                            // Add to Set button
                            Button(action: { showingAddToSet = true }) {
                                AddToSetCard()
                            }
                            .buttonStyle(.plain)
                        }
        ...
        .sheet(isPresented: $showingAddToSet) {
            AddSceneToSetView(scene: scene)
        }
        .sheet(isPresented: $showingAIAssistant) {
            TravelSceneAIAssistantView(scene: scene)
        }
    }
...
// MARK: - Add to Set Card

struct AddToSetCard: View {
    var body: some View {
        HStack(spacing: 12) {
            ZStack {
                RoundedRectangle(cornerRadius: 10)
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 14)
                .strokeBorder(.blue.opacity(0.3), lineWidth: 2, antialiased: true)
        )
    }
}
...
// MARK: - Add Scene to Set View

struct AddSceneToSetView: View {
    @Environment(ModelData.self) private var modelData
    @Environment(\.dismiss) private var dismiss
    let scene: TravelScene
            ...
            List {
                if modelData.sceneSets.isEmpty {
                    ContentUnavailableView {
                        Label("No Sets Available", systemImage: "folder.badge.plus")
                    } description: {
                        Text("Create a set first to organize your scenes")
                    }
                } else {
